...
void CTacticalVehicle::UpdateDynamics(double dElapsedTime_sec,stringstream& sstrMessage)
{
	...
	if(dDeltaTimeSec > 0.0)
	{
		...
		for(int iCount=0;iCount<iNumberUpdates;iCount++)
		{
			...
			case cmdVelocityHeadingAltitude:
				vdGetInputsVehicle()[CVEHICLESIMULATION_t::inapbvVelocity_fps] = vdGetCommands()[cmdvahVelocity_fps];
				vdGetInputsVehicle()[CVEHICLESIMULATION_t::inapbvPsi_rad] = vdGetCommands()[cmdvahHeading_rad];
				vdGetInputsVehicle()[CVEHICLESIMULATION_t::inapbvAltitude_ft] = vdGetCommands()[cmdvahAltitude_ft];
				break;
			}	//switch(cmdGetType())
