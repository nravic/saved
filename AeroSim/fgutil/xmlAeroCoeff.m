function Coeff = xmlAeroCoeff(ACTree, Idx)

% Function name:
% Coeff = xmlAeroCoeff(ACTree, Idx)
%
% Description:
% Load an aerodynamic coefficient into a Matlab coefficient structure
%
% Inputs:
% ACTree = the aircraft tree structure, as provided by xml_parser
% Idx = the UID of the coefficient element
%
% Outputs:
% Coeff = the coefficient structure (empty if TreeStruct is not an
...
% If the structure type is element and the name is coefficient then we have
% a valid coefficient
if strcmp(ACTree{Idx}.type, 'element') & strcmp(ACTree{Idx}.name, 'COEFFICIENT')
    % Save name
    ...
    if length(ContentUID) > 1
        error('Comments not supported inside an aerodynamic coefficient tag.');
    else
        ...
        if strcmp(ACTree{ContentUID}.type, 'chardata')
            ...
            % Next string is the set of non-dimensionalizing parameters
            ch = '|';
            while strcmp(ch, '|')
                [NonDimParams,count,errmsg,newidx] = sscanf(DataString(nextidx:end), '%s', 1);
                nextidx = nextidx + newidx;
                [ch, count, errmsg, someidx] = sscanf(DataString(nextidx:end), '%s', 1);
                if strcmp(ch, '|')
                    nextidx = nextidx + someidx;
                end
            end
